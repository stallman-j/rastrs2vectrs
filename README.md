...
1)  Copy the repository era5-extraction to your local machine
2)  Open `era5-extraction/code/00_startup.R`
...
c)  go into Copernicus and decide what information you actually want. I often just want the monthly mean precipitation which you can access below:

<https://cds.climate.copernicus.eu/cdsapp#!/dataset/reanalysis-era5-single-levels-monthly-means?tab=form>

Choose NETCdf since that's easier to bring into R with metadata. At the bottom click "Show API request" and copy that code

d)  paste this API request code into a .py file. Alternatively you can paste it into a .txt file which is saved as e.g. "import_era5_precip.py" and in files "All File Types". Save this into the folder which is your `code_download` folder (set in step 2).

if using more than one variable, rename the 'download.nc' to make clearer which name it is. The one I use for precipitation throughout is `total_precipitation.nc`. I'm sure you could do rename this through R (to do).
...
4)  Open `era5-extraction/code/01_download-datasets.R` and run the script. This will download 
